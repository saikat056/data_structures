...
=end
    def add_num(val)
      return if @map[val]
      floor = @map.floor_entry(val)
      ceil  = @map.ceiling_entry(val)
      
      return if floor && floor.value >= val
        
      if floor && ceil
        if (floor.value == (val - 1)) && (ceil.key == (val + 1))
          floor.value = ceil.value
          @map.delete(ceil.key)
        elsif (floor.value == (val - 1))
          floor.value = val
        elsif (ceil.key == (val + 1))
          @map.push(val, ceil.value)
          @map.delete(ceil.key)
        else
          @map.push(val, val)
        end
      elsif floor
        if (floor.value == (val - 1))
          floor.value = val
        else
          @map.push(val, val)
        end
