class Connect4
  ...
  def play
    ...
    while(turn < (@r_size*@c_size)) do
      ...
      col = (player == 1 ? get_valid_input : computer.get_turn(last[0], last[1]))
...
class Computer
  ...
  def get_turn(row, col)
    ...
    if row + 1 < m
      if @grid[row + 1][col] ==  @op_player
        return col
      else
        if (col - 1 >= 0) && (@grid[row][col - 1] == @op_player)
