class Connect4
  ...
  def initialize
    ...
    # S = O(m*n)
    @grid = Array.new(@r_size) { Array.new(@c_size) { EMPTY } }
    @move_hash = {}
    
    # S = O(n)
    (0..(@c_size - 1)).each do |i|
      @move_hash[i] = @r_size - 1
    end
  end
  ...
  def play
    ...
    while(turn < (@r_size*@c_size)) do
      ...
      if self.class.is_winner?(player, row, col, @grid)
  ...
  def self.win_cell_count(row, col, i, j, player, grid)
    count = 0
    m = grid.size
    n = grid.first.size
  ...
  # T = O(1)
  def self.is_winner?(player, row, col, grid)
    return true if win_cell_count(row, col, -1,  0, player, grid) + win_cell_count(row, col,  1,  0, player,  grid) >= 3 
    return true if win_cell_count(row, col,  0, -1, player, grid) + win_cell_count(row, col,  0,  1, player,  grid) >= 3 
    return true if win_cell_count(row, col, -1,  1, player, grid) + win_cell_count(row, col,  1, -1, player,  grid) >= 3 
    return true if win_cell_count(row, col,  1,  1, player, grid) + win_cell_count(row, col, -1, -1, player,  grid) >= 3 
  end
...
class Computer
  ...
  def get_turn_mini_max
    _, max_col = mini_max(Connect4::AI, Connect4::HUMAN, grid_clone, @move_hash.dup, 5)
    max_col
  end
  ...
  def mini_max(player, op_player, grid, mh, depth)
    ...
    (0..(n - 1)).each do |col|
      ...
      if Connect4.is_winner?(player, row, col, grid)
